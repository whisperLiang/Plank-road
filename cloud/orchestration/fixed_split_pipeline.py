...
from cloud.orchestration.proxy_stage import FixedSplitProxyEvaluator
from cloud.orchestration.runtime_stage import (
    FIXED_SPLIT_DYNAMIC_BATCH as _FIXED_SPLIT_DYNAMIC_BATCH,
...
from cloud.orchestration.settings import OrchestrationSettings
from cloud.orchestration.teacher_stage import TeacherAnnotationStage
from cloud.orchestration.training_stage import FixedSplitTrainingStage
...
from cloud.training.proxy_eval import (
    _is_cuda_oom_error,
    _looks_like_fused_ultralytics_state_dict,
    _coerce_positive_int,
    _normalise_shard_dtype,
    _normalise_label_schema,
    _normalise_class_name,
    _class_names_from_metadata,
    _label_name_from_schema,
    _infer_yolo_model_num_classes,
    _is_low_quality_trigger_sample,
    _runtime_image_size_from_metadata,
    _original_image_size_from_metadata,
    _runtime_input_tensor_shape_from_metadata,
    _pool_label_metadata_from_record,
    _splitter_dynamic_batch_range,
    _splitter_dynamic_batch_min,
)
...
class FixedSplitPipeline:
    ...
    def _move_runtime_input_to_device(value: object, device: torch.device):
        ...
        if isinstance(value, tuple):
            return tuple(
                FixedSplitPipeline._move_runtime_input_to_device(item, device)
                for item in value
            )
        if isinstance(value, list):
            return [
        ...
        if isinstance(value, dict):
            return {
                key: FixedSplitPipeline._move_runtime_input_to_device(item, device)
                for key, item in value.items()
            }
        return value
        ...
        *,
        ...
        with self._training_job_scope(edge_id):
            ...
            try:
                ...
                logger.info(
                    "[SamplePool] selection: before={} incoming={} kept={} "
                    "dropped={} dropped_high_quality={} dropped_low_quality={} "
                    "dropped_stale={} dropped_invalid={} deferred_feature_layout={}.",
                    selection_stats.get("before", 0),
                    selection_stats.get("incoming", 0),
                    selection_stats.get("kept", 0),
                    selection_stats.get("dropped", 0),
                    selection_stats.get("dropped_high_quality", 0),
                    selection_stats.get("dropped_low_quality", 0),
                    selection_stats.get("dropped_stale", 0),
                    selection_stats.get("dropped_invalid", 0),
                    selection_stats.get("deferred_feature_layout", 0),
                )
                logger.info(
...
# Backward-compatible import name for local tools/tests; cloud_server exposes the same alias.
CloudContinualLearner = FixedSplitPipeline
