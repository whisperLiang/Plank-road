...
from cloud.training.proxy_eval import (
    ...
    _proxy_prediction_cache_threshold_low,
    _proxy_metrics_skipped_full_proxy,
    _is_cuda_oom_error,
    ...
    _set_detection_model_eval_mode,
...
class CloudFixedSplitOrchestrator:
    ...
    def __init__(self, config, large_object_detection: Object_Detection):
        ...
        self.weight_folder = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "model_management", "models"
        )
    ...
    def _move_runtime_input_to_device(value: object, device: torch.device):
        ...
        if isinstance(value, tuple):
            return tuple(
                CloudFixedSplitOrchestrator._move_runtime_input_to_device(item, device)
                for item in value
            )
        if isinstance(value, list):
            return [
        ...
        if isinstance(value, dict):
            return {
                key: CloudFixedSplitOrchestrator._move_runtime_input_to_device(item, device)
                for key, item in value.items()
            }
        return value
