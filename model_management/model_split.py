...
def load_feature_cache(cache_path: str, frame_index: int) -> dict:
    ...
    # Feature cache payloads contain Python/numpy metadata in addition to
    # tensors. In PyTorch >=2.6, torch.load defaults to weights_only=True,
    # which rejects such payloads unless allowlisted. Cache files here are
    # generated by our own pipeline, so trusted deserialisation is expected.
