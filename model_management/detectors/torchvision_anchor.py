...
TorchvisionAnchorDetectorReplay = _split.TorchvisionAnchorDetectorReplay
_TORCHVISION_BUILTIN = _zoo._TORCHVISION_BUILTIN
_TORCHVISION_WEIGHT_DEFAULTS = _zoo._TORCHVISION_WEIGHT_DEFAULTS

build_anchor_detector_training_loss = _split.build_anchor_detector_training_loss
build_ssd_split_training_loss = _split.build_ssd_split_training_loss


class TorchvisionAnchorBackend(DetectionBackend):
    family = "torchvision_anchor"

    def matches_name(self, name: str) -> bool:
        return _zoo._normalise_model_name(name) in _TORCHVISION_BUILTIN

    def matches_model(self, model: torch.nn.Module) -> bool:
        if isinstance(model, TorchvisionAnchorDetectorReplay):
            return True
        return _split._is_anchor_detector(model)

    def list_model_names(self) -> list[str]:
        return list(_TORCHVISION_BUILTIN)
...
BACKEND = TorchvisionAnchorBackend()
