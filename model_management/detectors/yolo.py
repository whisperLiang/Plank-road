...
YOLODetectionModel = _zoo.YOLODetectionModel
_YOLO_MODELS = _zoo._YOLO_MODELS

infer_ultralytics_state_dict_num_classes = _zoo.infer_ultralytics_state_dict_num_classes
...
class YOLOBackend(DetectionBackend):
    ...
    def matches_name(self, name: str) -> bool:
        return _zoo._normalise_model_name(name) in _YOLO_MODELS

    def matches_model(self, model: torch.nn.Module) -> bool:
        return isinstance(model, YOLODetectionModel)

    def list_model_names(self) -> list[str]:
        return list(_YOLO_MODELS)
