...
_TINYNEXT_MODELS = _zoo._TINYNEXT_MODELS
_TINYNEXT_REPO_MODELS = _zoo._TINYNEXT_REPO_MODELS

infer_tinynext_state_dict_num_classes = _zoo.infer_tinynext_state_dict_num_classes
_remap_tinynext_public_detections = _zoo._remap_tinynext_public_detections
_configure_tinynext_label_schema = _zoo._configure_tinynext_label_schema


class TinyNeXtBackend(DetectionBackend):
    family = "tinynext"

    def matches_name(self, name: str) -> bool:
        return _zoo._normalise_model_name(name) in _TINYNEXT_MODELS

    def matches_model(self, model: torch.nn.Module) -> bool:
        if isinstance(model, _split.TorchvisionAnchorDetectorReplay):
            model = model.detector
        return bool(getattr(model, "_plank_public_label_hook_registered", False)) or (
            str(getattr(model, "label_schema", "")).strip().lower() in {"coco_91", "zero_based"}
            and hasattr(model, "num_classes")
            and type(model).__name__ == "SSD"
        )

    def list_model_names(self) -> list[str]:
        return list(_TINYNEXT_MODELS)
